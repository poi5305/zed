...
impl Fs for RemoteFs {
    ...
    fn path_exists(&self, path: &std::path::Path) -> bool {
        // No sync RPC exists for existence. Unknown paths return false so the
        // worktree watcher treats them as pending rather than watching a path
        // that may not be there.
        if lock_shared(&self.prefetched_metadata).contains_key(path) {
            return true;
        }
        let directories = lock_shared(&self.prefetched_directories);
        if directories.contains_key(path) {
            return true;
        }
        directories
            .values()
            .any(|entries| entries.iter().any(|entry| entry == path))
    }

    fn is_path_case_sensitive(&self, _path: &std::path::Path) -> bool {
        // No per-path sync RPC. Returning true (case-sensitive) matches typical
        // Linux servers and avoids folding `Foo`/`foo` into one watch key, which
        // would merge distinct files on a case-sensitive volume. A miss on a
        // case-insensitive volume is conservative; a false merge is not.
        true
    }

    fn requires_poll_watcher(&self, _path: &std::path::Path) -> bool {
        // RemoteFs::watch already delivers server-side native events over RPC.
        // Returning true would defer that watch and poll path_exists locally,
        // which we cannot answer honestly without blocking the wasm thread.
        false
    }
