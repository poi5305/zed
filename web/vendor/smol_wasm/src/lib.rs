...
//!
//! On native targets this crate re-exports the real `smol` crate. On
//! `wasm32-unknown-unknown` it keeps the pure-Rust pieces (`channel`, `lock`,
//! `future`, …) and replaces OS-dependent modules so the crate does not depend
//! on `async-io` or `async-process`.
