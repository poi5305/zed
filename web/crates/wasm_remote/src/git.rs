...
use git::repository::{
    ...
    RepoPath, ResetMode, SearchCommitArgs, Upstream, UpstreamTracking, UpstreamTrackingStatus,
    Worktree,
};
...
#[derive(Deserialize)]
struct GitCommitTemplateResponse {
    template: String,
}
...
impl GitRepository for RemoteGitRepository {
    ...
    fn load_commit(
        ...
        async move {
            ...
            Ok(CommitDiff {
                ...
                // The current RPC returns only the file list. `true` would hide
                // those diffs behind the shallow-history banner. `false` is the
                // safe default until the server returns the flag.
    ...
    fn load_commit_template(&self) -> BoxFuture<'_, Result<Option<GitCommitTemplate>>> {
        ...
        let repo_path = self.repo_path.clone();
        async move {
            let response: Option<GitCommitTemplateResponse> = client
                .call(
                    "GitRepository::load_commit_template",
                    &json!({ "repo_path": repo_path.to_string_lossy() }),
                )
                .await?;
            Ok(response.map(|response| GitCommitTemplate {
    ...
    fn commit_data_reader(&self) -> Result<CommitDataReader> {
        // `from_async_resolver` was removed. CommitDataReader's fields are
        // private and `for_test` is test-only with a sync callback, so the
        // existing `GitRepository::commit_data` RPC cannot be wrapped here.
        Err(anyhow!(
            "commit_data_reader is not supported over the web RPC bridge: \
             CommitDataReader has no public constructor"
        ))
    }
