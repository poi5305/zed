...
fn is_shallow_boundary_commit(fs: &FsRpc, params: &Value, commit: &str) -> Result<bool> {
    let Ok(common_dir) = git_text(fs, params, &["rev-parse", "--git-common-dir"]) else {
        return Ok(false);
    };
    ...
    let shallow_path = if common.is_absolute() {
        common.join("shallow")
    } else {
        directory.join(common).join("shallow")
    };
    let Ok(contents) = fs::read_to_string(shallow_path) else {
        return Ok(false);
    };
    let spec = format!("{commit}^{{commit}}");
    let oid = git_text(fs, params, &["rev-parse", "--verify", &spec])?;
    Ok(contents.lines().any(|line| line.trim() == oid.trim()))
}
...
fn load_commit(fs: &FsRpc, params: &Value) -> Result<Value> {
    let ignore_shallow_boundary = params
        ...
        .unwrap_or(true);
    let commit = string(params, "commit");
    if !ignore_shallow_boundary && is_shallow_boundary_commit(fs, params, commit)? {
        // Keep a JSON array so wasm_remote can still deserialize
        // Vec<CommitFileResponse>. Coordinating an
        // `{ files, is_shallow_boundary }` body is a later client change.
        return Ok(Value::Array(Vec::new()));
    }
    let output = git(
        fs,
